...
def getWeather():
    try:
        ...
        #weather
        api = 'https://api.openweathermap.org/data/2.5/weather?q='+city+'&appid=7ef7776ee3a63dbdc05d537f458a1d10'

        json_data = requests.get(api).json()
        condition = json_data['weather'][0]['main']
        temp = round(json_data['main']['temp']-273)
        temperature_feels = round(json_data['main']['feels_like']-273)
        pressure = int(json_data['main']['pressure']/1.333)
        humidity = json_data['main']['humidity']
        wind = json_data['wind']['speed']

        t.config(text=(temp,'°'))
        c.config(text=(translator.translate(condition),'|', 'ОЩУЩАЕТСЯ', 'КАК:', temperature_feels, '°'))
